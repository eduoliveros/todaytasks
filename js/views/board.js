...
export function TodayTasksBoardView(ctx){
  ...
  const _formatDateFriendly = (d) => (window.TodayTasksUtils && window.TodayTasksUtils.formatDateFriendly) ? window.TodayTasksUtils.formatDateFriendly(d) : formatDateFriendly(d);
  ...
  function renderBoard(schedule){
    ...
    if(titleEl){
      if(!isToday){
        titleEl.textContent = `Planificación para el ${_formatDateFriendly(state.selectedDate)} (${state.selectedDate})`;
      } else {
    ...
    if(badgeEl){
      badgeEl.textContent = state.planningMode
        ? "inicio " + _fmt(state.workStart)
        : (isToday ? "ahora " + _fmt(now) : state.selectedDate);
    }
    ...
    if(visible.length === 0){
      const emptyMsg = state.workEnd !== null && state.workEnd !== undefined
        ? 'No hay reuniones ni tareas programadas hasta el fin de jornada (' + _fmt(state.workEnd) + ').'
        : 'No hay reuniones ni tareas programadas hoy (día libre).';
      el.innerHTML = '<div class="board-empty">' + _escapeHtml(emptyMsg) + '</div>';
      return;
    }
    ...
    for(const e of visible){
      ...
      const label = e.kind === "meeting" ? "🗓 " + _escapeHtml(e.label)
                  : e.kind === "buffer" ? "colchón · " + _escapeHtml(e.label)
                  : _escapeHtml(e.label) + (e.kind==="task-running" ? " (en curso)" : "");
      html += `
        <div class="slot ${kindClass}">
          <span class="time-label">${_fmt(start)}<span class="sep">–</span>${_fmt(e.end)}</span>
          <span>${label}</span>
          <span class="dur">${_fmtDur(e.end-start)}</span>
        </div>`;
    }
    if (state.workEnd !== null && state.workEnd !== undefined) {
      html += `<div class="slot" style="opacity:.6;border:1px dashed var(--board-line);background:none;">
          <span class="time-label">${_fmt(state.workEnd)}</span>
  ...
  function renderSummary(schedule){
    ...
    if (meetingsEl) {
      meetingsEl.innerHTML = meetings.length ? meetings.map(m => `
        <div class="summary-row">
          <div class="row-top"><span>${_escapeHtml(m.title)}</span></div>
          <div class="time-range tr-meeting"><span class="tag">Inicio</span>${_fmt(m.start)}<span class="arrow">→</span><span class="tag">Fin</span>${_fmt(m.end)}</div>
        </div>
    ...
    if (completedEl) {
      const completedRows = completed.map(t => {
        ...
        return `
        <div class="summary-row">
          <div class="row-top"><span>${_escapeHtml(t.title)}</span><div style="text-align:right"><span class="dur task-duration-clickable" title="Clic para ajustar tiempo consumido" onclick="app.openTimePopover('${_escapeAttr(t.id)}', event)">${_fmtDur(t.actualDuration)} reales</span> <span class="dur" style="opacity:0.75">(plan. ${_fmtDur(t.planned)})</span></div></div>
          <div class="time-range tr-running"><span class="tag">Inicio</span>${_fmt(realStart)}<span class="arrow">→</span><span class="tag">Fin</span>${_fmt(t.completedAt)}</div>
      ...
      const interruptionRows = interruptions.map(i => `
        <div class="summary-row summary-row-interruption">
          <div class="row-top">
            <span>⚡ ${_escapeHtml(i.title)}</span>
            <span class="dur" style="color:var(--danger)">${_fmtDur(i.duration)}</span>
          </div>
          <div class="time-range tr-interruption">
            <span class="tag">Inicio</span>${_fmt(i.start)}
            <span class="arrow">→</span>
            <span class="tag">Fin</span>${_fmt(i.end)}
          </div>
        </div>`);
    ...
    if (pendingEl) {
      pendingEl.innerHTML = pending.length ? pending.map(t => {
        ...
        if(t.status === "running"){
          ...
          const chip = `<span class="remaining-chip${rem.overrun?" overrun":""}">${_escapeHtml(rem.text)}</span>`;
          rangeHtml = `<div class="time-range tr-running"><span class="tag">Inicio real</span>${_fmt(t.runningStart)}<span class="arrow">→</span><span class="tag">Fin previsto</span>${_fmt(plannedEnd)} ${chip}</div>`;
        } else {
          ...
          if(segs.length){
            ...
            rangeHtml = `<div class="time-range ${trClass}"><span class="tag">Inicio est.</span>${_fmt(segs[0].start)}<span class="arrow">→</span><span class="tag">Fin est.</span>${_fmt(segs[segs.length-1].end)}</div>`;
          } else {
        ...
        return `
        <div class="summary-row">
          <div class="row-top"><span>${_escapeHtml(t.title)}${autoMoveTag}</span><span class="dur">${_fmtDur(t.planned)} plan. · <span class="task-duration-clickable" title="Clic para ajustar tiempo consumido" onclick="app.openTimePopover('${_escapeAttr(t.id)}', event)">${elapsedReal > 0 ? _fmtDur(elapsedReal) : '0m'} realizados</span> · ${t.status === 'paused' ? 'en pausa' : t.status}</span></div>
          ${rangeHtml}
...
if (typeof window !== "undefined") {
  window._TodayTasksBoardView = TodayTasksBoardView;
}
