...
export function computeMetricsFromDay(dayData) {
  ...
  const meetingsTime = computeOccupiedMeetingTime(meetings);
...
export function snapshotAndPrune(state) {
...
export function saveHistoryMetric(state, dateStr, metrics) {
...
export function deleteHistoryMetric(state, dateStr) {
  const envKey = state.activeEnv || "work";
  const env = state.environments[envKey];
...
export function renderHistoryView(ctx) {
