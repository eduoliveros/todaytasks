...
* **Resiliencia ante el Ciclo de Vida Móvil y Reconexión (`v1.112` / [ADR 020](./adr/020-resiliencia-sincronizacion-ciclo-de-vida-movil.md)):** Para evitar que tareas creadas o modificadas en smartphones queden atascadas cuando el sistema operativo suspende la pestaña o apaga la pantalla, `TodayTasksCloud` escucha activamente los eventos del ciclo de vida web:
  - `visibilitychange (hidden)`: Descarga y envía inmediatamente a Firestore cualquier cambio encolado en el debounce (`flushPendingCloudPush`) antes de la congelación del proceso móvil.
  - `visibilitychange (visible)` y `focus`: Reactiva de forma defensiva la red de Firestore (`fbDb.enableNetwork()`) despertando sockets latentes o zombis (*half-open*), vacía escrituras pendientes y restaura el estado visual si había ocurrido un error previo.
  - `online`: Reconecta automáticamente la red de Firestore y reanuda la sincronización tan pronto como el dispositivo recupera cobertura WiFi o móvil.
...
* **Carga Inicial Robusta y Fallback ante Conexiones Congeladas:** Cuando un cliente se conecta por primera vez sin datos en `localStorage`, Firestore puede retrasar o no emitir el primer snapshot del servidor (especialmente bajo auto-detección de long-polling o redes restrictivas). `TodayTasksCloud` cuenta con un temporizador de fallback (5s) que invoca `cloudDocRef(uid).get()` para asegurar la descarga del documento remoto. Tras la carga o combinación inicial de datos remotos, se dispara el callback `onCloudDataLoaded` (coordinado en `app.js`), que ejecuta automáticamente `actionsModule.materializeRecurringTasks()` y `actionsModule.rolloverPendingTasks()` para materializar tareas periódicas y trasladar tareas pendientes de jornadas pasadas a hoy, actualizando la interfaz sin requerir recargar la página.
...
* **Reordenación y Movimiento en Bloque de Tareas Multiseleccionadas (`v1.111` / [ADR 019](./adr/019-triaje-reordenacion-masiva-bloque-seleccionado.md)):**
  - **Arrastre en Bloque (PC y Touch):** Cuando el usuario tiene varias tareas seleccionadas y arrastra una de ellas, todas las tareas seleccionadas se desplazan juntas como un único bloque contiguo a la nueva posición, conservando estrictamente su orden relativo original.
    - *Soltar sobre tarea no seleccionada:* Si se arrastra hacia abajo (`fromIdx < toIdx`), el bloque se posiciona tras el objetivo; si es hacia arriba (`fromIdx > toIdx`), se posiciona antes del objetivo desplazándolo hacia abajo.
    - *Soltar sobre una tarea de la misma selección:* Todas las seleccionadas se compactan y agrupan ordenadamente en torno a la tarea objetivo.
    - *Arrastrar tarea no seleccionada:* Mueve únicamente dicha tarea sin afectar a las seleccionadas ni alterar la selección.
    - *Feedback visual e invariantes:* Todas las tareas seleccionadas se iluminan con `.dragging` durante el arrastre y permanecen seleccionadas tras soltar para encadenar acciones.
  - **Controles de Posición en Barra Flotante (`#triageFloatingBar`):** Botones directos `⤒` (Inicio), `▲` (Subir), `▼` (Bajar) y `⤓` (Fin) para desplazar el bloque de seleccionadas.
  - **Bottom Sheet Móvil y Botones Rápidos de Fecha:** Al accionar dirección en el Bottom Sheet móvil o pulsar un botón rápido de fecha en una tarea seleccionada, la acción se propaga a todo el grupo seleccionado.
* **Operaciones Masivas y Barra Flotante (`#triageFloatingBar`):** Permite selección múltiple de tareas (individual o por grupo completo) y ofrece mover a cualquiera de los próximos 7 días laborables, mover posición (inicio, arriba, abajo, fin), cambiar urgencia en lote, destacar en lote, completar en lote (`✓ Completar`) o borrar en lote con confirmación y soporte transaccional de Undo (`Ctrl+Z`).
...
* **Drag & Drop Táctil y Ratón por Long-Press (`touch-drag.js`):** ([ADR 022](./adr/022-drag-drop-tactil-long-press.md) y [ADR 025](./adr/025-drag-drop-hibrido-raton-long-press.md))
  - Motor compartido `createTouchDragEngine(options)` que encapsula el *state machine* de pulsación prolongada (420 ms para toques táctiles, 250 ms para ratón `mouseHoldDelay`, 60 ms sobre la manija `⠿`), vibración háptica, detección de objetivo vía `document.elementFromPoint` y supresión del *click* sintético posterior (`wasRecentTouchDrag()`).
  - Durante el arrastre con ratón, vincula listeners globales de `mousemove` y `mouseup` a nivel de `window` y desactiva temporalmente el atributo nativo `draggable` para evitar interferencias.
  - Usado tanto por **triaje** (`.triage-task-row`, con soporte de multiselección) como por el **tablero principal** (`.task-item`, tarea única) reutilizando `reorderTaskByDrag`. En vista móvil/estrecha ($\le$ 640px) la manija `⠿` se oculta para mantener el diseño compacto; el arrastre se activa manteniendo pulsada la celda tanto con el dedo en dispositivos táctiles como con el botón izquierdo del ratón en PC.
