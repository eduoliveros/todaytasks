...
describe('Weekly Schedule and Day Navigation', () => {
  ...
  it('actualiza inicio y fin de jornada según el día de la semana', () => {
    ...
    expect(window.TodayTasksUtils.fmt(state.workStart)).toBe('09:00');
    expect(window.TodayTasksUtils.fmt(state.workEnd)).toBe('17:00');
    ...
    expect(window.TodayTasksUtils.fmt(state.workStart)).toBe('09:00');
    expect(window.TodayTasksUtils.fmt(state.workEnd)).toBe('15:00');

    // Sábado (2026-08-15, día libre)
    state.selectedDate = '2026-08-15';
    expect(state.workStart).toBeNull();
    expect(state.workEnd).toBeNull();
    expect(window.TodayTasksUtils.fmt(state.workStart)).toBe('');
    expect(window.TodayTasksUtils.fmt(state.workEnd)).toBe('');
  });
  ...
  it('respeta la anulación (override) manual de horas en un día concreto', () => {
    ...
    expect(window.TodayTasksUtils.fmt(state.workEnd)).toBe('16:00');
